...
import { TaskModal } from 'src/sections/admin/notification/task-modal';
import { ColumnCard } from 'src/sections/admin/notification/column-card';
import { ColumnAdd } from 'src/sections/admin/notification/column-add';
import { useDispatch, useSelector } from 'src/store';
import { thunks } from 'src/thunks/notification';

const useColumnsIds = () => {
  const { columns } = useSelector((state) => state.notification);
...
const useBoard = () => {
  ...
  const handleBoardGet = useCallback(async () => {
    await dispatch(thunks.getBoard());
  }, [dispatch]);
...
const Page = () => {
  ...
  return (
    <>
      <Seo title="Dashboard: Notification" />
      <Box
        component="main"
        sx={{
          display: 'flex',
